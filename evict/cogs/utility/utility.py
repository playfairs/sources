...
class Utility(Extended, Cog):
    ...
    async def afk_listener(self, ctx: Context) -> Optional[Message]:
        ...
        if len(ctx.message.mentions) == 1:
            ...
            rate_key = xxh64_hexdigest(f"{ctx.channel.id}:{user.id}")
    ...
    async def avatarhistory(self, ctx: Context, user: Optional[Member] = None) -> Message:
        ...
        if not avatars:
            return await ctx.warn(f"No avatar history found for {user.mention}")
        ...
        try:
            ...
            embed = Embed()
            embed.set_author(name=f"{user}'s Avatar History", icon_url=user.display_avatar)
            ...
            if total_count > 16:
                embed.description = f"There is {total_count} avatars. Clear with `{ctx.clean_prefix}avh clear`"
                embed.set_footer(text=f"Showing 16/{total_count} avatars")
            else:
                embed.description = f"There is {total_count} avatars. Clear with `{ctx.clean_prefix}avh clear`"
                embed.set_footer(text=f"Total avatars: {total_count}")
    ...
    async def avatarhistory_clear(self, ctx: Context) -> Message:
        ...
        try:
            ...
            return await ctx.approve("Your avatar history has been deleted")
